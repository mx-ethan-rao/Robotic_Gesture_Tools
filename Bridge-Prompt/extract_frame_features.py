...
def main():
    ...
    if args.dataset == 'breakfast':
        ...
        val_data = JIGSAWS_FRAMES(transforms=transform_val, save_feat_dir=config.data.save_dir)
    else:
        val_data = None
        ...
        for iii, (image, filename) in enumerate(tqdm(val_loader)):
            if not os.path.exists(os.path.join(save_dir, filename[0])):
                ...
                if non_splt:
                    ...
                    for s_img, s_filepath in zip(image_features, filename):
                        os.makedirs(s_filepath.rsplit('/', 1)[0], exist_ok=True)
                        np.save(s_filepath, s_img.contiguous().cpu().numpy())
