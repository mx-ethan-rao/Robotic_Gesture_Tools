...
class Unstack(object):
    ...
    def __call__(self, stacked_img):
        # Convert the input to numpy array if it's a tensor
        if isinstance(stacked_img, torch.Tensor):
            stacked_img = stacked_img.cpu().numpy()

        # Check if the image is RGB or grayscale based on the channel size
        channel_len = 3 if stacked_img.shape[0] % 3 == 0 else 1
        num_images = stacked_img.shape[0] // channel_len

        # Split the channels to retrieve the original sequence of images
        img_group = [stacked_img[i * channel_len : (i+1) * channel_len, :, :] for i in range(num_images)]

        # If the original Stack function had the 'roll' operation to reverse RGB channels,
        # we need to reverse it back in the Unstack function
        if self.roll and channel_len == 3:
            img_group = [img[::-1, :, :] for img in img_group]

        # Convert list of numpy arrays back to a single PyTorch tensor
        img_tensor = torch.stack([torch.from_numpy(img) for img in img_group])
